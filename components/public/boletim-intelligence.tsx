...
export function BoletimIntelligence() {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      setStatus(result.success ? { type: 'success', message: result.message } : { type: 'error', message: result.message })
                 ...
                 <div className="md:col-span-9 p-6 sm:p-8 space-y-3 sm:space-y-4">
                    ...
                    <div className="pt-3 sm:pt-4 flex items-center justify-between border-t border-border/40">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-tighter text-dark">{noticia.impacto}</span>
                       </div>
         ...
         <div className="bg-white border border-border rounded-3xl p-6 sm:p-8 text-center space-y-4 shadow-sm">
            ...
            <div className="space-y-2">
               <div className="flex gap-2">
                  ...
                  />
                  <button 
                    className={`btn-primary w-10 h-10 p-0 shrink-0 shadow-none ${loading ? 'opacity-50' : ''}`}
               ...
               {status && (
                 <p className={`text-[8px] font-black uppercase tracking-tighter ${status.type === 'success' ? 'text-secondary' : 'text-primary'}`}>
                   {status.message}
                 </p>
               )}
            </div>
