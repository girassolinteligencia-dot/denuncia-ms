...
export async function registrarDenuncia(
  ...
  const montarDescricaoAnonima = (
    ...
    const partes = [
      ...
      data.agente_politico === 'sim' ? `Cargo do agente político: ${data.cargo_agente || 'Não informado'}` : null,
  ...
  try {
    ...
    const { data: denuncia, error: denErr } = await supabase
      ...
      .insert({
        ...
        cargo_agente:       formData.agente_politico === 'sim' ? formData.cargo_agente || null : null,
    ...
    try {
      ...
      const pdfBuffer = await gerarPDFDenuncia({
        ...
        cargo_agente:     formData.agente_politico === 'sim' ? formData.cargo_agente || '' : '',
      ...
      if (catData?.email_destino) {
        ...
        sendEmail({
          ...
          html: gerarEmailOrgao({
            ...
            nome: formData.is_anonima ? undefined : formData.nome,
            email: formData.is_anonima ? undefined : emailNorm,
            telefone: formData.is_anonima ? undefined : formData.telefone,
            cpf: formData.is_anonima ? undefined : formData.cpf,
